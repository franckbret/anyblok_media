...
class TestMediaVideoModel:
    """ Test media video model"""

    def init_registry(self, func):
        self.registry = init_registry_with_bloks(("media-video",), func)
    ...
    def test_media_video_file_with_storage_strategy_disk(self):
        filename = "sample.mp4"
        dir_path = os.path.join(os.path.dirname(__file__), "media_sample/")
        source_path = dir_path + filename
        assert self.registry.Media.Video.get_source_storage_strategy() == "disk"

        with open(source_path, "rb") as fp:
            data = fp.read()
            media = self.registry.Media.Video.create(file=data, filename=filename)
    ...
    def test_media_video_file_path(self):
        filename = "sample.mp4"
        dir_path = os.path.join(os.path.dirname(__file__), "media_sample/")
        source_path = dir_path + filename
        tag = self.registry.Tag.insert(name="sample", media_type="video")
        media = self.registry.Media.Video.create(
